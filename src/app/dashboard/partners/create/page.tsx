...
import { createBanner, type BannerData } from "@/src/lib/api/partners";
...
export default function CreateBannerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      console.log("Отправка данных Партнерыа:", formData);
      await createBanner(formData);
      toast({
        title: "Успешно",
        description: "Партнеры успешно создан",
      });
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Не удалось создать Партнеры",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      >
        <ArrowLeft className="mr-2 h-4 w-4" /> Назад к Партнеры
      </Button>
      ...
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label>Изображение Партнерыа</Label>
              <FileUpload
                multiple={false}
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={loading}>
                {loading ? "Создание..." : "Создать Партнеры"}
              </Button>
            </div>
          </form>
        </CardContent>
