...
export default function BannersPage() {
  ...
  const fetchBanners = async () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Не удалось получить Новостиы",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Успешно",
        description: "Новости успешно удален",
      });
      fetchBanners();
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Не удалось удалить Новости",
      });
    } finally {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">Список Новости</h2>
        <Button onClick={() => router.push("/dashboard/news/create")}>
          <Plus className="mr-2 h-4 w-4" /> Добавить Новости
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-8">
          <p className="text-muted-foreground">
            Новостиы не найдены. Создайте первый Новости.
          </p>
        </div>
      ) : (
        <Table>
              ...
              <TableRow key={banner._id}>
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    size="icon"
                    onClick={() =>
                      router.push(`/dashboard/news/edit/${banner._id}`)
                    }
                  >
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Это действие необратимо. Новости будет удален навсегда.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
