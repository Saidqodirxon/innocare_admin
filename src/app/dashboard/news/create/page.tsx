...
import { createBanner, type BannerData } from "@/src/lib/api/news";
import type { ImageData } from "@/src/lib/api/news";
...
export default function CreateBannerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      console.log("Отправка данных Новостиа:", formData);
      await createBanner(formData);
      toast({
        title: "Успешно",
        description: "Новости успешно создан",
      });
      ...
      toast({
        variant: "destructive",
        title: "Ошибка",
        description: error.message || "Не удалось создать Новости",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      >
        <ArrowLeft className="mr-2 h-4 w-4" /> Назад к Новости
      </Button>
      ...
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label>Изображение Новостиа</Label>
              <FileUpload
                multiple={false}
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={loading}>
                {loading ? "Создание..." : "Создать Новости"}
              </Button>
            </div>
          </form>
        </CardContent>
